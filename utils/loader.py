...
class DatasetTool(object):
    ...
    def load_data(data_path):
        ...
        # We assume that each data file is formatted as domain_XXXXX.json,
        # so that the domain could be identified by the file name.
        data = []
        for dialogue_components_item in raw_data:
            kb,history,query, last_response, consistency = DatasetTool.get_info(dialogue_components_item, domain)
            data_item = dict()
            data_item["knowledge_base"] = kb
            data_item["history"]=history
    ...
    def get_info(dialogue_components_item, domain):
        ...
        # Get the <dialogue history> and the <last response>
        sentences = []
        history_sentences = []
        last_response = ''
        ...
        # print(dialogue_components_item)

        # Construct(Concat and add special token)
        # the <history_sentences>,<last_response>,<kb_expanded>
        kb,history,query = DatasetTool.construct_info(kb_expanded, history_sentences)

        return kb,history,query, last_response, consistency
    ...
    def expand_kb(knowledge_base, domain):
        ...
        if domain == "navigate":
            ...
            for kb_row in knowledge_base:
                ...
                entity = kb_row['event']
                for attribute_key in kb_row.keys():
                    # if attribute_key == "event":
                    #     continue
                    # We won't add no-value pair into representation
    ...
    def construct_info(kb_expanded, history_sentences):
        ...
        for row in kb_expanded:
            kb += ' '.join([triple[1] + " " + triple[2] for triple in row])
            kb += ' ; '
        kb += ' [EOK] '
        history = ''
        # construct_info = ''
        ...
        # construct_info_list = []
        # for row in kb_expanded:
        #     construct_info_list.append(' '.join([triple[1] + " " + triple[2] for triple in row]))
        # construct_info += ' ; '.join(construct_info_list)
        # construct_info += ' [EOK] '

        # Construct and add history_sentences
        for i, sentence in enumerate(history_sentences):
            if i % 2 == 0:
                history += " [USR] " + sentence
            else:
                history += " [SYS] " + sentence
        query=history_sentences[-1]
        return kb,history,query
